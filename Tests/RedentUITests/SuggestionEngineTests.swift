...
struct SuggestionEngineTests {
    ...
    func emptyQuery(_ query: String) async {
        #expect(await makeEngine().suggestions(for: query, engine: .duckduckgo).isEmpty)
    }

    @Test("Typing a host offers to open it directly, first")
    func directURLFirst() async {
        let rows = await makeEngine().suggestions(for: "example.com", engine: .duckduckgo)
    ...
    func searchRowIsLast() async throws {
        ...
        ).suggestions(for: "swift", engine: .duckduckgo)
    ...
    func respectsLimit() async throws {
        ...
        let rows = await makeEngine(history: entries)
            .suggestions(for: "site", engine: .duckduckgo, limit: 6)
        #expect(rows.count <= 6)
    }
}
...
private struct FakeBookmarkStore: BookmarkStoring {
    ...
    func all() async -> [Bookmark] { items }
    func favorites() async -> [Bookmark] { items.filter(\.isFavorite) }
    func search(_ query: String, limit: Int) async -> [Bookmark] {
        Array(items.filter { $0.displayTitle.lowercased().contains(query.lowercased()) }.prefix(limit))
    }
    func bookmark(for url: URL) async -> Bookmark? { items.first { $0.url == url } }
